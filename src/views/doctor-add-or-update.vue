...
export default {
    ...
    methods: {
		...
		//然后定义一个初始化函数，对模型层变量visible 设置成true ，浏览器就能弹出对话框了。
		init:function(id) {
			let that = this;
			//先重置表单控件
			that.reset();
			...
			that.$nextTick(()=>{
				...
				if(that.dataForm.id){
					that.$http('/doctor/searchById','POST',{id:id},true,function(resp) {
						...
						//这一行是把数据库中存储的数字 先转换成字符的数字，然后用我们定义的json进行映射
			...
			that.$refs['dataForm'].validate(function(valid){
				if(valid){
					...
					let data = {
						...
						//前端传过来的是汉字，我们需要转换成的数字存储进去
						recommended: that.dataForm.recommended == '推荐' ? 1 : 2,
						status: JSON1[that.dataForm.status],
